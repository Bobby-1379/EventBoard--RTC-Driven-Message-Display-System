...
#include "setting.h"         // Custom settings function
...
u32 msg_flag = 1,temp;             // Flag to control message display
...
int main()
{
    ...
    SetRTCTimeInfo(13, 44, 55);
    // Set initial RTC date (15 July 2025)
    SetRTCDateInfo(12, 12, 2026);
    // Set RTC day (Tuesday)
    SetRTCDay(2);
        ...
        for (i = 0; i < TOTAL_MESSAGES; i++)
        {
            if (messageList[i].hour == hour && messageList[i].minute == min && messageList[i].enabled == 1)
            {
                ...
                while (1)
                {
                    ...
                    msg_scroll(messageList[i].text, len, 100); // Scroll message slowly
                }
