...
void Init_ADC(void)
{
    cfgPortPin(0, 28, AN0_INPUT_PIN);  // P0.28 = AD0.1

    ADCR = (1 << 21) | (4 << 8);          // clock divider
}/*
...
{
	/*
    ADCR &= ~(0xFF);          // clear channel bits

   // ADCR |= (1 << 21);        // ensure ADC ON

    ADCR |= (1 << chNo);      // select channel
...
u32 Read_LM35()
{
 ...
 /* u32 tdegC;      // Variable to store temperature in degree Celsius (integer)
  static u32 flag=0;  // Static flag to ensure ADC is initialized only once
...
*/
  ...
  return  (u32)(eAR * 100);          // Convert voltage to temperature in Celsius (10mV per degree, LM35 output)
